...
| F39 | E2 | **MACI / coercion-resistant member voting** | 🟡 | `open_maci_round(coordinator, challenge_secs)` + `MaciState` `["macistate", round]`; ZK sign-up commit–reveal (`maci_signup_commit`, `maci_signup`, `MaciSignup`/`MaciSignupCommit`, `member_vote` reused under a domain-separated external nullifier); `publish_maci_message` (unchanged); `close_maci_round` (permissionless freeze), `process_maci_messages` (permissionless crank folding an on-chain `chain_digest` in strict index order); `commit_maci_tally` / `finalize_maci_round` **present but DISABLED**; `frontend/lib/maci-command.ts` (command format, state machine, audit digests — pure) + `frontend/lib/maci.ts`; `tests/maci-machine.ts`, `tests/maci.ts` (wired into `anchor test`); `docs/maci.md` | **Processing half shipped; tallying deliberately disabled.** The queue's completeness, order, non-replay and electorate are now enforced BY THE PROGRAM, and a key change verifiably voids a coerced ballot. What the chain still cannot do is verify the tally itself (needs process/tally circuits ⇒ F44), so `commit_maci_tally` and `finalize_maci_round` return `MaciTallyUnverified` and `finalize_maci_round` never writes `MemberProposal.passed` — closing the privilege-escalation path Sentinel raised as CRITICAL (NRR-2026-08-12-f60-f61-maci): an unverified tally must not reach `install_elected_seat` / `refill_faucet`. Results are computed and audited **off chain** (`coordinatorTally`) against the on-chain digests. Remaining: F44 circuits + ceremony, MPC coordinator, relay-allowlist entries for the two sign-up instructions, voting UI. |
...
| F35 | E0 | **Gas faucet — first gas for the neophyte** | ✅ ⚠ | `init_faucet` (any seat), `set_faucet_amount` (Treasurer, ≤ on-chain cap), **`activate_faucet_zk` (anonymous, preferred)**, `activate_faucet` (named, **deprecated**), `refill_faucet` (passed member vote, one-shot); `FaucetJar` (`state.rs:623-639`); `docs/faucet.md`; `tests/faucet.ts` (21 cases); `frontend/lib/faucet.ts`, `frontend/lib/zk-vote.ts` (`proveWingEndorsement`, F35-R2; `proveMemberEndorsement` still serves `attest_admission_zk`), `tests/sentinel/f35r2-wing-gate.sh` | Per-Circle jar PDA `["faucet", circle]`. Parrain attestation = the **F27** WingPeer bond; **one grant per membership commitment per Circle** via nullifier `["faucetnull", circle, commitment]` (E0's story says "one grant, one time, ever" — fellowship-wide dedup would need a linkable commitment, so it is delegated to **F5**). Uniform grant `jar.grant_lamports`, cap `FAUCET_MAX_GRANT_LAMPORTS = 2_000_000` (`state.rs:643`); a retune pauses grants for `FAUCET_AMOUNT_COOLDOWN` 24h so an amount cannot be aimed at one neophyte; refill capped at `FAUCET_MAX_REFILL_GRANTS = 100` grants' worth. Refill binds to an F6 vote whose `description_hash = hashv("AHA-faucet-refill" ‖ circle ‖ amount_le)` (solana `hashv` — sha256). **Sponsor edge removed (2026-08-12f).** `activate_faucet_zk` proves the endorsement with the shipped `member_vote` VK — external nullifier `SHA-256("AHA-faucet-grant" ‖ circle ‖ neophyte)` masked into BN254 (domain-separated from `attest_admission_zk`'s, so one member's two anonymous acts never share a nullifier), `choice = 1`, relayed via F55. **F35-R2 (this round):** the proof no longer says "SOME member of the tree" — its `root` must equal `single_leaf_root(wing_peer.wing)`, a depth-20 single-leaf root the PROGRAM computes from the bond, so only the holder of the wing's secret can produce it (`EndorsementNotByWing`). **Mandatory sponsorship restored with no new circuit and no new ceremony — F44 out of scope**; the check REPLACES the current-or-F54-recent root gate (joining them with `||` would leave the hole exactly as wide) and the instruction shape, account count (10) and payerIndex (8) are unchanged, so `relayPolicy.ts` needs no edit. **No parrain account of any kind is in the transaction**; both paths share `pay_uniform_grant` and the same `["faucetnull", circle, commitment]` one-shot, so the economics are byte-identical and the forms cannot be stacked. The named path is kept, deprecated in code + docs, ONLY as the fallback for a parrain whose device holds no ZK voting key; the client prefers the anonymous path (`haveVotingKey`). ⚠ **Residual limits (post-F35-R2):** ~~the proof says *a member*, not *the designated wing*~~ and ~~a neophyte already in the member tree can endorse their OWN grant~~ — **both closed by F35-R2**; the claim that either needed **F44** was wrong and is refuted in `docs/faucet.md`. What F35-R2 costs instead: the endorsement's commitment-level anonymity set is now **1** (the `root` argument is a deterministic public function of `wing_peer.wing`, so the wing acting at that slot becomes a *record* rather than the *guess* the public `WingPeer` PDA already allowed — **F27** / Sentinel R2, and F27 must retire this check together with the bond or it regresses by arithmetic); the wing is no longer proved to be in the member tree, so an expired/revoked/not-reinserted — and now also a **provisional** — commitment can endorse (parity with the named path; descoped, documented); and the wing becomes a single point of failure for a one-shot grant, with the deprecated named path the only fallback. Still true: the bound is one grant per commitment and the membership door; both paths still reject `owner == default`, so first gas needs a bound wallet; an open+no-personhood Circle can still farm its own jar (bounded by the membership door, not the endorsement). Remaining E0 work: **F49** (F47/F48 shipped same day, pilot form); real-proof e2e for the anonymous path is uncovered, same gap as **F53**. ⬜ *No devnet upgrade signature recorded — every other shipped row cites one.* |
...
| F34 | E10 | **Foundation-led federation governance** (rotate seats / delete a Circle, 4-of-7) | ✅ shipped — ⚠️ **CRITICAL fund-safety hole, proven & unfixed** | `propose/approve/execute_child_rotation`, `propose/approve/execute_child_close` (`validity_secs`, 1–90-day window), `ChildSeatVote`/`ChildCloseVote`; `frontend/app/foundation/`, `lib/foundation.ts` | the foundation Council can rotate any federation Circle's 7 seats or delete a Circle (closes it + delists its profile). Federation = Circles sharing the foundation's root `parent`. This already makes some cross-Circle votes **binding** — see the sovereignty ADR under *Open decisions*. **⚠️ KNOWN HOLE (2026-08-15, CRITICAL, pre-existing, not yet fixed):** `execute_child_close` closes the Circle account but never checks or sweeps the separate `["treasury", circle]` PDA. The funds survive but become unreachable (every spend path needs the now-closed Circle to deserialise), and because a Circle is a PDA of `(parent, name)` and `initialize_circle` is permissionless with caller-chosen seats, a stranger can re-register the same name, seat themselves 4-of-7 and drain it. Proven end-to-end in `tests/treasury-orphan.ts` (6/6, local validator only — **never run on devnet, it moves funds**) and reproduced independently by the `f59-presence` Sentinel round from a clean deploy. Three devnet treasuries are reachable today (0.1/0.205/0.08 SOL). Full write-up + fix options in `docs/testing-foundation.md` §0; cheapest fix (require `treasury.lamports == 0` in `execute_child_close`) is a **governance decision awaiting the user**, deliberately not auto-applied. |
...
| F59 | E4 | ZK presence attestation ("last stood in circle: March 2026") | 🟡 **on-chain shipped** (2026-08-15) | `attest_presence_zk` + `clear_presence` wired into `lib.rs`; `Presence` PDA (13 bytes, one overwritten scalar); `month.rs` coarsens `Clock` to a closed-month index on chain. No new circuit and no new ceremony — both proofs reuse the shipped `member_vote` key, so **F44 stays out of scope**. Design of record `docs/presence.md` (includes the Epic 4 amendment and its accepted ledger-archaeology residual). 36/36 Rust tests, 12/12 real-proof tests (`tests/presence-zk.test.mjs`). **Remaining:** browser prover, the QR handoff screen (which must disclose that it reveals the subject's raw commitment), and the presence line on `/member` — nothing in the UI reads a `Presence` account yet. |
...
| # | Epic | Item | Status | Notes |
|---|------|------|--------|-------|
| F92 | E5/E2 | Circle-creation preconditions (sponsor validation + rent) | ✅ (2026-08-12) | `/create` no longer renders the wizard to anyone who cannot finish it. TWO gates, checked on arrival and never conflated: (1) **sponsor validation** — the wallet must hold an ACTIVE membership with NO `ProvisionalMember` marker, i.e. a parrain attested AND a trusted servant confirmed (Epic 2's asymmetric pair). A provisional newcomer gets a blocking popup saying it is too early and their sponsor must validate them first; founding a Circle seats its creator on a 7-seat Council, so an unvalidated founder is a governance hole, not a cosmetic one. The sponsor check fails CLOSED on RPC error. (2) **rent** — `initialize_circle` + `initialize_member_tree` open two rent-exempt PDAs (the 20-level member tree is the expensive one); the wallet's balance is compared against `getMinimumBalanceForRentExemption` for both plus a 0.03 SOL fee headroom, and an underfunded member is told the exact figures BEFORE filling in seven seats rather than hitting a raw "insufficient lamports" halfway through. Fails OPEN on RPC error (the runtime still refuses an underfunded tx). `frontend/lib/createGate.ts`, `frontend/app/create/page.tsx`, `.gate-*` CSS. i18n: 8 keys × 19 locales. |
| F93 | E12 | "Get AHA" install page + platform-aware nav entry | 🟡 (2026-08-12: page + nav shipped; no published build to point at) | Nav shows **"Get AHA for iOS"** on iPhone/iPad (including iPadOS, which reports a desktop Mac UA — caught via `maxTouchPoints`) and **"Get AHA for Android"** everywhere else, linking to `/get-app`. The page states the position honestly instead of showing dead store badges: AHA is on **neither Google Play nor the App Store**; Android has a real installable debug-signed APK from the manual `mobile.yml` CI workflow (and downloading a workflow artifact needs a GitHub account with repo access — stated on the page); **iOS has no installable build at all**, because CI produces an unsigned archive and iOS will not install unsigned code without an Apple Developer signing identity — iOS visitors are given the Add-to-Home-Screen route instead, which is close to what the v1 remote-load shell does anyway. `frontend/lib/platform.ts` (shared with the wallet chooser), `frontend/app/get-app/page.tsx`. **Remaining**: page BODY is English-only (nav labels are translated ×19) — queue for the next i18n pass; and the real unblock is signing credentials + store listings (see `docs/mobile.md` checklist), which are the user's own and are deliberately not in this repo. |
| F94 | E9 | **Glossary — searchable/browsable reference under a renamed Resources menu** | 🟢 (2026-08-14) | `frontend/app/glossary/page.tsx`; `scripts/build-glossary.mjs` → `frontend/public/glossary.json`; source of record `glossary/glossary_v1.xlsx`; `tests/sentinel/glossary-check.sh`; nav menu "The 12" → **Resources** (`ResourcesMenu` in `Nav.tsx`) | 342 terms across seven merged source collections (Ancestral, Shamanic, Celtic, Hinduism, Andean, Polynesian, Sacred Numbers), each cross-tagged to a Step. Search over term/explanation/tags with in-place highlighting; facets by Step 1–12 and by tradition; A–Z index; the spreadsheet's "related to" column rendered as live cross-references; `?q=` deep links. The extractor parses the OOXML zip by hand (no new dependency) and matches columns BY HEADER NAME, so reordering columns cannot silently shift data — Sentinel round `glossary` verified this by writing an independent XML reader and confirming byte-identical regeneration. **Privacy:** no chain call, no wallet, no member data; one static fetch, so which terms a member reads is not observable. ⚠ Two spreadsheet content defects reproduced faithfully rather than silently corrected: `Budhism`/`Buddhism` exist as two tags, and `Freemassonery` is misspelled — fix at source, re-run the extractor. |
| F95 | E12 | **Mobile wallet connect — the escape hatch for browsers that cannot finish MWA** | 🟢 (2026-08-14) | `frontend/lib/mobileWallet.ts`; `frontend/components/MobileWalletNotice.tsx`; `browse` field in `docs/wallets.json` + `frontend/public/wallets.json`; `frontend/e2e/mobile-wallet.spec.ts` + `mobile-firefox-android` Playwright project; `tests/sentinel/e12-wallet-check.sh` §8c | Reported from a Samsung Z Fold on Firefox for Android, and reproduced with both Solflare and Phantom — which is the tell that it is not the wallet. `@solana/wallet-adapter-react` injects a `SolanaMobileWalletAdapter` on its own (we pass an empty adapter array); its environment check is only "UA says Android and this is not a WebView", so it offers MWA to **every** Android browser. MWA needs the browser to fire a `solana-wallet:` intent and hold an association open across the app switch — Chrome does, Firefox does not, so the wallet opens cold on its account screen and the tab spins forever. **No client-side fix exists for the handshake**; the browser is what cannot carry it. So the banner states that BEFORE the tap and offers to reopen the site inside the wallet's own in-app browser, where the wallet injects and Wallet Standard registration works normally. **T6:** the offered wallets are read from `wallets.json` and shuffled with the same uniform Fisher–Yates as `WalletChooser` (Sentinel measured 200,000 iterations at 50.16/49.84); a wallet appears only if it publishes a documented `browse` universal link, and the copy says so rather than letting a short list read as a recommendation. **Privacy:** the user agent chooses a hint and nothing else — no identifier derived, nothing stored, nothing transmitted, one same-origin static fetch. ⚠ **Residual, disclosed not closed:** a captioned randomised 2-of-5 subset still means only Phantom and Solflare ever get a one-tap button (Sentinel's own reading, and I agree with it). ⚠ Body copy is English-only, matching the F93 precedent — queue for the next i18n pass. ⚠ The e2e project emulates the DETECTION path on Chromium; it cannot prove the Gecko handshake fails, which stays a real-device property. |
| F96 | E12 | **The connect button, the cluster box, and the banner's third tone** | 🟢 (2026-08-15) | `frontend/components/WalletButton.tsx`; `frontend/components/NetworkSelector.tsx`; `noticeTone()` in `frontend/lib/mobileWallet.ts`; `frontend/e2e/wallet-button.spec.ts`, `network-selector.spec.ts`, `wallet-notice-tone.spec.ts` | Three user-visible changes. **Label:** upstream's `WalletMultiButton` hardcodes its LABELS, so this uses `BaseWalletMultiButton` with an explicit set; only `no-wallet` is re-worded ("Select Wallet" → "Connect"). **Mark:** the Solana logo moves into the button as a CSS `::before`, because `BaseWalletConnectionButton` overwrites `startIcon` unconditionally; a `:not(:has(...))` guard stops two icons stacking once a wallet is selected. **Testnet:** dropped from the cluster box — AHA is not deployed there — but removed *conditionally*, because a `<select>` whose value matches no option renders BLANK, and a control whose job is to name your chain must never go blank. **Third tone:** the banner told iPhone users the in-app browser was "the most reliable way"; on iOS it is the **only** way, since the mobile adapter injects on Android alone. `noticeTone()` now returns `broken` / `only-route` / `alternative` from one place so the component cannot drift from the rule. Covered by `NRR-2026-08-15-connect-and-cluster.md` (PASS WITH WARNINGS), which chose its own mutations rather than replaying the author's. ⚠ **Known limit, recorded not fixed:** `isIOS()`'s touch-Mac heuristic misclassifies a desktop Mac with a touchscreen display; iPadOS impersonates macOS deliberately and every tightening that excludes the Mac also excludes real iPads, which is the worse error. |
| F97 | E5 · E2 | **Sponsorship UX on /me — Sponsors & Sponsees blocks, compassionate wording** | 🟢 (2026-08-15, `af41220`) | `frontend/app/me/page.tsx` (reorder + reframed MentorshipCard); `frontend/app/sponsor-request/page.tsx`; `frontend/components/InboxNavLink.tsx`; `.inbox-badge` in `globals.css`; `me.spon.*` + `sponreq.*` in `i18n.generated.ts`; `tests/sentinel/sponsor-wording-check.sh`; `tests/badge-count.test.mjs`; `frontend/e2e/sponsor-request.spec.ts` | `/me` reordered to **My memberships → Join a Circle → Sponsors → Sponsees**. Compassionate wording throughout — "Release this Link", "Accept this Link", "Not at this time"; **"bond" renamed "Link"** per the user (2026-08-15). Both release paths confirm first and state that the other person keeps full use of the platform and can still create Circles. The Sponsees list is now the real one (it previously showed only the subset still eligible for the first-gas grant). `/sponsor-request` is the QR/link landing page; the QR is drawn locally via the already-vendored `qrcode` dep and the link carries only the Circle address and the inviter's **already-public** commitment. Messages badge: top-right red circle, hidden at 0, `99+` cap, `aria-label="My Messages, 3 unread"`; nav label "My Inbox" → "My Messages". **Sponsor graph is PRIVATE per the user's decision** — a member sees only their own links; the round confirmed no new enumeration path (the pre-existing F27 WingPeer PDA enumerability is unchanged, not deepened). ⚠ **Direction correction:** only the mentee may write a WingPeer link (`establish_wing_peer.rs` — "a member sets their own wing, never imposed"), so a scanned invite necessarily makes the **scanner the sponsee**; the copy now says "take someone under your wing". The reverse direction (asking someone above you, they accept) needs a two-way handshake and is **still open**. ⚠ **Two pieces of coverage that could not fail were caught and replaced in the same commit** — an e2e test asserting "never says reject" PASSED with "Reject" live (the control only renders for a connected wallet), and the badge had no coverage at all. Replaced by a static wording gate and pure-function tests, both mutation-verified red. ⚠ Copy is English-only (keys in the `en` block; other locales inherit via the fallback chain) — queue for the next i18n pass. Covered by `NRR-2026-08-15-f97-sponsorship.md` (PASS WITH WARNINGS). |
| F98 | E2 · E5 | **Sponsorship karma (ranks members — Tradition 2 waived)** | 🟢 on-chain (2026-08-15) — ⬜ no UI | `state::Karma` / `KarmaParams` / `KarmaAward`; `ProposalAction::SetKarmaParams` + `instructions/set_karma_params.rs`; credit inside `establish_wing_peer`; `tests/karma.ts` | Built under an **explicit written waiver** (CLAUDE.md, 2026-08-15): *"I waive the Tradition 2 no-ranking rule for F98 karma, and accept that it ranks members."* `karma` was removed from `privacy-sweep.sh`'s forbidden identifiers; its siblings (score/rating/ranking/reputation/leaderboard/streak) remain forbidden, and **Tradition 2's own text is untouched** — "no ranking" was an engineering invariant derived from it, and only the derived rule was retired. Accepting a Link credits the sponsee `gain_sponsee` (default 100) and the sponsor `gain × sponsor_ratio_bps/10000` (default 10%); basis points, not floats. A Circle's numbers are set by an executed 4-of-7 `SetKarmaParams`; the first Link materialises the defaults on chain so karma works with no governance step. `min_sponsors` is stored but **ADVISORY** — a test asserts a member with no sponsor still acts. ⚠ **A CRITICAL was found by the `f98-karma` round and fixed in the same session:** the `KarmaAward` guard was seeded in ROLE order, so the same two members could swap roles, derive a second award and collect twice (reproduced live: 220 between them instead of 110). The seed is now the **canonicalised (sorted) pair**, and `tests/karma.ts` asserts the pair's *combined* total after a swap — the assertion the original test lacked. ⚠ **Publicly comparable by design:** a `Karma` PDA derives from a commitment and commitments are enumerable, so anyone can build a ranked table of a Circle. That is what the waiver accepts. Onboarding copy saying so before a member earns a point is **not yet written**. ⬜ No UI surfaces karma yet. |
| F62 | E6 | Stone-mark canvas + private, deliberately-disclosed avatar | 🟢 | Drawing canvas for a personal sign inside an equilateral triangle, as an alternative to photo upload (only upload exists — `frontend/lib/profile.ts` `fileToAvatarDataUrl`, canvas resize to 128 px). Move the avatar out of localStorage into the **F60** object; add audience selection; render **one identical neutral silhouette** when not disclosed, never a lock or placeholder that signals hiddenness. Off-device storage must be ciphertext only — no public IPFS gateway. **Done 2026-08-12:** the canvas existed but was incomplete AND carried three real defects, all fixed: an **unbalanced canvas state stack** (`ctx.save()` on stroke start / `restore()` on end leaked a frame permanently on pointercancel or lost capture, and `clear()` never restored), ink chosen from `prefers-color-scheme` when this app's theme lives on `data-theme` — so it never followed the app and **baked pale ink into exports made in dark mode**, invisible on another member's light-themed page — and an export that resampled the *screen* canvas, making output device-pixel-ratio dependent. Now rendered from normalised geometry: unit-space strokes, `clampToTriangle` treating the triangle as a **bound on recorded geometry** (a stroke is cut at the edge and restarts on re-entry, not merely clipped), adjustable stroke, undo, and a deterministic export ladder (128px PNG → WebP q0.92 → 96px → 64px) under a **24 KiB** data-URL cap. `/me` now offers two equal, non-preselected routes — draw a mark or upload a photo — each clearing the other's artefact. **Privacy:** stroke *dynamics* (timing, velocity, pressure, tilt) are never recorded and even the geometry is discarded at save (only the raster persists), so a mark cannot be replayed as a handwriting biometric; `setStoneMark` refuses SVG outright since the avatar renders into an `<img src>` on other members' pages. 17/17 tests incl. a 4000-point fuzz proving every clamped point lands inside the stone. |
...
| # | Epic | Item | Status | Notes |
|---|------|------|--------|-------|
| F67 | E9 | `docs/wallets.json` + shuffled WalletChooser | 🟢 (2026-08-14) | `docs/wallets.json` (five self-custodial wallets, mirrored byte-identical to `frontend/public/wallets.json`), `frontend/components/WalletChooser.tsx` (uniform Fisher–Yates on every mount, `Math.random()` only, never seeded by any user identifier), rendered on `/onboarding` step 1 AND `/create` | **The component shipped earlier; the defect this row named did not get fixed until now.** `/create` still had two hardcoded anchors, Solflare then Phantom, in fixed order — the wrong count AND a permanent first position (T6) — and `/settings-security` named the same two in body copy. Removed in `b217a62` / `0717f6e`. Sentinel round `f67` stress-tested the shuffle at 200,000 iterations: max deviation from flat 20% = 1.00%, so no wallet is favoured. T6 sweep now clean: every remaining wallet brand mention in `frontend/` is a code comment, none is member-visible copy. |
| F68 | E9 | `/onboarding` — three-step illustrated stepper | 🟢 (verified 2026-08-14) | `frontend/app/onboarding/page.tsx` (Wallet → Vouch → Face, ~297 lines), linked from `frontend/components/Nav.tsx`; `docs/onboarding.md` | **This row was stale, not the code**: the stepper was built and wired. Sentinel round `nav-menu` confirmed it exists and is reachable BEFORE this row was reconciled to that claim. F69 cartoonisation is still deferred and labelled as such in the UI. Since 2026-08-14 the link also sits in the Resources menu, so it stays reachable for connected members after the "Start Here" nav entry retires. |
| F69 | E9 | On-device photo cartoonisation | 🟢 | Recognisable to the circle, useless to face recognition. **Assert that no original photograph reaches any storage, upload or log** — F33 today stores the resized raw photograph as a data URL in localStorage. **Done 2026-08-12:** `frontend/lib/cartoonise.ts`. **The finding that mattered:** `fileToAvatarDataUrl` persisted a real, unmodified photograph merely resized to 128px — and a 128px face crop is ample for off-the-shelf face recognition, so downscaling was never a privacy transform. It also materialised the **full-resolution** photo twice in memory (`FileReader.readAsDataURL` → base64 string → `img.src` → decoder cache), never released. Now: decoded straight from the Blob via `createImageBitmap` (no FileReader, no base64, no data URL), drawn once into a **detached** canvas which is then cleared and sized to 0×0 with the bitmap `close()`d, and every intermediate buffer — each of which still resembles the face — **zero-filled** before the promise resolves, on the success path and again in `finally`. Algorithm is classical and dependency-free: cover-crop → saturation lift → bilateral-ish edge-preserving smoothing using rational weights `1/(1+d²/σ²)` (**no `Math.exp`**, so only `+ - * /` and therefore bit-identical across engines) → Sobel L1 ink mask from the smoothed image → **median-cut** palette to 8–22 colours, splitting at the **midpoint of the axis range** rather than the population median (splitting at the median lost the eyes; a frequency-ranked palette merged pupils into the cheek and turned skin green) → ink applied last. Deterministic, no randomness or clock. Output 1.3–2.3 KB at 128px against a 48 KiB cap. **Honest limit, stated in `docs/avatars.md`:** this destroys fine texture and landmark precision and defeats naive matching, but is **not** a proof against a determined adversary holding the original; and the guarantee covers this app only — not the member's own disk, the OS picker, or device backups. 45/45 tests including determinism, dark-outlier survival (the eyes), buffer zeroing, and six source-level invariants that fail the suite if `fetch`/storage/`FileReader`/`toDataURL` reappear. |
...
| R8 | MACI advertised coercion resistance it could not deliver | **F39** → E2 / E10 | **partly closed 2026-08-12.** The processing half is real and program-enforced (chain digest, one-member-one-voice ZK sign-up, freeze, replay/reorder guards) and `docs/maci.md` now states the threat model, the defeated and NOT-defeated coercion scenarios, and the coordinator trust assumption in the present tense. The tally half is **disabled on chain** rather than over-claimed. Fully closes with F44. |
