...
| F97 | E5 · E2 | **Sponsorship UX on /me — Sponsors & Sponsees blocks, compassionate wording** | ⬜ requested 2026-08-15 | (proposed) `frontend/app/me/page.tsx` reorder; new Sponsors/Sponsees sections; QR + link invite; built on the EXISTING F27 WingPeer bond, not new on-chain infra | User request (verbatim spec preserved in this row). Reorder `/me` to: **My memberships → Join a Circle (moved directly under memberships) → Sponsors → Sponsees**. **Sponsees** block lists people you sponsor with an "End this sponsorship / Release this Link" action (compassionate wording — user renamed "bond"→"**Link**" 2026-08-15) behind a gentle confirm; sponsor can end unilaterally, sponsee cannot veto, history stays visible. **Sponsors** block lists your active + pending upper sponsors, a "recommends at least {minSponsorsRequired} sponsors" soft hint, and invite flows: **Share QR code** (deep link `/sponsor-request?to=…&token=…`) for face-to-face and **Copy invitation link** for private message; a scan/opened link lands on an accept page with **"Accept this Link" / "Not at this time"** (compassionate, never "reject"). This maps onto **F27** (`establish_wing_peer` set by mentee, `end_wing_peer` by either party, `WingPeer` PDA `["wingpeer", circle, mentee]`) — the relationship already exists on-chain and is already endable by both sides. **⚠ Traditions caveat carried from F27, must be resolved before build:** the WingPeer PDA already publishes a commitment→commitment **sponsor edge** — *"exactly what E2 exists to remove"* (Sentinel R2). Surfacing it as named+avatared Sponsor/Sponsee lists deepens that public social graph. Options for the user (see F98): keep the lists **private to the member** (a member sees only their own edges, no global enumeration) vs. the fully public graph the spec sketches. The compassionate wording, the /me reorder, the QR/link invite, and the "soft-minimum" hint carry no Traditions conflict and can ship regardless. |
