...
- **Deploy-cost:** `opt-level = "z"` (from `"s"`) + `strip` + `anchor-spl`
  ...
  ~110 KB). Program-account rent ≈ **8.28 SOL** (was ≈ 8.88). Biggest *account*-rent
  win — `Message::CT_LEN 1040→528` (~0.0036 SOL/message) — is deferred: it needs a
  client re-pad + a product call on max message length, and F32 is E7-sunset-bound.
