...
| F98 | E2 · E5 | **⛔ Karma / sponsorship-points system — CONFLICTS WITH A LOCKED POSITION** | ⬜ requested 2026-08-15 — **needs an explicit written waiver to build as specified** | (proposed) `Member.karma`, `karmaGainSponsee`, `karmaGainSponsorRatio`, `minSponsorsRequired`, Foundation-Council-editable `SystemParameters` | User request: on accepting a sponsorship, credit the sponsee `karmaGainSponsee` (≈100) and the sponsor a ratio of it (≈10%), all editable by Foundation Council vote; enforce a soft `minSponsorsRequired` (default 2). **⛔ This is the single most directly-forbidden construct in the project.** A per-person karma total is a **reputation score that ranks one member above another (Tradition 2)**; `tests/sentinel/privacy-sweep.sh` greps for `karma|score|rating|rank|reputation|tier|badge` and **fails the build** on a hit, and `docs/presence.md` states *"any feature that introduces a per-person counter is a CRITICAL."* Building it as written would auto-FAIL the mandatory Sentinel gate and requires, per CLAUDE.md, **an explicit written waiver from the user citing the backlog line, in the commit note.** Non-ranking alternatives that satisfy the same intent without a score: (a) drop points entirely — sponsorship is its own acknowledgement; (b) a **private, non-comparative** "thank-you"/acknowledgement a member sees only on their own page, with **no number, no total, no ordering** and no cross-member visibility. `minSponsorsRequired` as a *soft, non-gating* hint carries no conflict on its own; only the karma numbers do. **No code until the user chooses.** |
