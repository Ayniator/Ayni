...
| F74 | E11 | In-person device-to-device handover | ⬜ (decided medium; **QR/NFC device code not yet built** — the custody + sharding layers have no network path, so handover is the remaining hardware piece) | **BOTH QR and NFC** (decided v0.2), at a circle meeting; the member uses whichever the two devices support. A shard must **never** appear in a request body, server relay, cloud backup, or log — built so there is **no network code path** a shard could take (for either medium), not a promised-unused one. |
