...
Decisions (resolved v0.2, per ElectaZ):
- **Challenge-window length = 7 days** (the working default), configurable by the fellowship but **never shortenable by any client-supplied parameter**. Critically, **"publish a recovery intent" must NOT become an on-chain, member-linkable event** — the intent lives in the off-chain shard-custody layer (a signal the sponsors' custody holds and the member's device can watch and cancel), never as an anchor transaction that ties a recovery to a member. This preserves the epic's core invariant: recovery emits nothing on chain, and even the *fact* that a recovery is pending is not publicly correlatable to an identity.
- **Shard-transfer medium = both QR and NFC.** Device-to-device, in person, at a circle meeting; the member chooses whichever the two devices support. Neither path may touch a network — no request body, relay, cloud backup, or log for either medium.
