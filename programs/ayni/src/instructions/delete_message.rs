...
/// Delete a message (reclaim its rent). With a sealed sender the chain no longer
/// knows who sent it, so the recipient may delete it anytime; once expired,
/// anyone may clean it up.
pub fn delete_message(ctx: Context<DeleteMessage>) -> Result<()> {
    ...
    require!(who == m.recipient || expired, AyniError::Unauthorized);
    Ok(())
}
