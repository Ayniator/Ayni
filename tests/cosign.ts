...
describe("ayni — member co-signature & self-recovery", () => {
  ...
  it("blocks council-only migration of a require_cosign membership, allows it with the guardian", async () => {
    ...
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(guardian.publicKey, 1e9)
    );

    // require_cosign = true, guardian = recovery_key
    await program.methods
      .issueMembership([...commitment], memberOwner, guardian.publicKey, true)
    ...
    // With the guardian co-signing, it succeeds.
    await program.methods
      .recoverMembership()
      .accounts({ circle: circlePda, proposal, membership, memberAuthority: guardian.publicKey, payer: authority.publicKey })
      .signers([guardian])
      .rpc();

    const m = await program.account.membership.fetch(membership);
    assert.ok(m.owner.equals(newWallet), "owner migrated with guardian co-signature");
  });
  ...
  it("lets a member self-migrate with a key they hold (no council)", async () => {
    ...
    await program.methods
      .issueMembership([...commitment], owner.publicKey, anchor.web3.PublicKey.default, false)
      .accounts({ circle: circlePda, membership, authority: authority.publicKey })
      .rpc();
